...
class communication_helper
{
	...
	function sendEmail1($to, $subject, $message, $fromEmailId, $attachment = array(), $is_attachment_array = 0, $ccEmailId = "", $ccAdmin = "")
	{
		...
		if (strlen($attachment) || $is_attachment_array) {
			...
			$headers .= "From: " . trim($fromEmailId) . "\r\n";

			if (!empty($ccEmailId)) {
				$headers .= "Cc: " . trim($ccEmailId) . "\r\n";
			}

			if (!empty($ccAdmin)) {
				$headers .= "Bcc: " . trim($ccAdmin) . "\r\n";
			}

			$headers .= "Reply-To: " . trim($fromEmailId) . "\r\n";

			$sent = mail($to, $subject, stripslashes($message), $headers);
		}
	...
	function sendEmail($to, $subject, $message, $fromEmailId)
	{
		global $cfg_loggingOn;

		try {

			$this->mailmgr->setFromAddress($fromEmailId, "Easy Math Building Blocks");
			$this->mailmgr->setToAddress($to);
			$this->mailmgr->setSubject($subject);
			$this->mailmgr->setBody($message);

			$sent = $this->mailmgr->sendEmail();

			if (!$sent && $cfg_loggingOn) {
				$this->logmgr->writeToLog(
					'communication_helper',
					"SMTP send failed to EmailId:" . $to,
					logMgr::$LG_ERROR
				);
			}
	...
	function sendResetPasswordEmail($fromEmailId, $toEmailId, $new_password, $username)
	{
		...
		$subject = "Easy Math Building Blocks - new password";
		...
		if (!$this->sendEmail($toAddress, $subject, $msg, $fromEmailId, $attachment)) {
			if ($cfg_loggingOn) {
				...
				$this->logmgr->writeToLog('communication_helper', "Failed to send email to EmailId:" . $toEmailId, logMgr::$LG_ERROR);
			}
		}
	}
	...
	private function createResetPasswordMessageForUser($fromEmailId, $toEmailId, $new_password, $username)
	{
		...
		// $_app_images_dir_url  = $_SESSION['EMRP_BASE_DIR'] . "app/views/images";
		// $_app_bootstrapdir_url = isset($_app_bootstrapdir_url) ? $_app_bootstrapdir_url : '/app';

		$_app_images_dir_url  = $_SESSION['EMRP_BASE_DIR'] . "app/views/images";
		$_app_bootstrapdir_url = $_SESSION['EMRP_BASE_DIR'] . "app";
		...
		$tempmsg = '<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<div style="margin-left: auto; margin-right: auto; width: 956px; height: 100%; background-color: #FFFFFF; text-align: left; border:0px solid #B4DD65;">
				...
				<div style="width:956px; height:80px;">	
				<img src="' . $_app_images_dir_url . '/logo.gif" width="200px;" height="80px" />
				<img src="' . $_app_bootstrapdir_url . '/assets/img/logo.png" alt="Logo" width="200px;" height="80px">
				</div>
				<!-- logo section end here -->
				<div style="width:956px; font-size:1px; height:10px; background-color:#0090FF;"></div>
				...
				<div style="width:956px; height:100%;">
					...
					<div align="center" style="width:910px; margin:20px; border:0px solid red;">
						<div align="left"  style="font-size:18px; font-weight:bold; color:#0090FF;">Easy Math Building Blocks new password </div>
						<div style="border-bottom:1px solid #0090FF;"></div><br />
