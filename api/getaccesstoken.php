...
if ($childname && $deviceid && $grade) {
    ...
    $accesstoken = getAccessTokenForChildForDeviceAndGrade($childname, $deviceid, $grade);

    if (!$accesstoken) {
        $responsedata = array(
            'status' => "failed",
            'description' => "Failed to retrieve access token for the Child for the given name, deviceid and grade ($childname, $deviceid, $grade)."
            //    'description' => "Failed to retrieve access token for the Child for the device ($childname, $deviceid)."
        );
        $em = new exceptionMgr(" ");
        $em->logInfo("getaccesstoken: Error: Failed to retrieve access token for the Child for the given name, deviceid and grade ($childname, $deviceid, $grade).");
    } else {
        $responsedata = array(
            'status' => "success",
