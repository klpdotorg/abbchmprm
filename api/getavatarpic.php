...
if ($data) {
    ...
    if ($childname && $deviceid && $grade) {
        ...
        if (!$objChild) {
            $responsedata = array(
                'status' => "failed",
                'description' => "Given combination of name and device and grade ($childname, $deviceid, $grade)."
                // 'description' => "Given combination of name and device ($childname, $deviceid) does not exist."
            );
            $em = new exceptionMgr(" ");
            $em->logInfo("getavatarpic: Error: Given combination of name and device and grade ($childname, $deviceid, $grade).");
        } else {

            $avatarpic_base64string = getAvatarpicEncodedStringByChildId($objChild->getChildId());
