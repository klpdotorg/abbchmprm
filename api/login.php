...
if ($childname && $deviceid && $grade) {
    ...
    $childid = login($childname, $deviceid, $grade);

    if (!$childid) {

        $responsedata = array(
            'status' => "failed",
            'description' => "No Child exists with the given name and device and grade ($childname, $deviceid, $grade)."
        );
        $em = new exceptionMgr(" ");
        $em->logInfo("login: Error: No Child exists with the given name and device and grade ($childname, $deviceid, $grade).");
    } else {
