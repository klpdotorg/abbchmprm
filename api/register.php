...
if ($data) {
    ...
    if ($childname && $deviceid && $grade && $language) {
        ...
        $childexists = checkIfNameDeviceAndGradeRegistered($childname, $deviceid, $grade);
        if ($childexists) {
            $responsedata = array(
                'status' => "failed",
                'description' => "Given combination of name, deviceid and grade ($childname, $deviceid, $grade) is already registered."
                // 'description' => "Given combination of name and deviceid ($childname, $deviceid) is already registered."
            );
            $em = new exceptionMgr(" ");
            $em->logInfo("register: Error: Given combination of name, deviceid and grade ($childname, $deviceid, $grade) is already registered.");
        } else {
            ...
            if ($rtn) {
                ...
                $childid = getChildIdByNameDeviceAndGrade($childname, $deviceid, $grade);
