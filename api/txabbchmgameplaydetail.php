...
if (count($jsondata_array, 1) == 0) {
    ...
    foreach ($jsondata_array as $data) {
        ...
        if ($data) {
            ...
            $childexists = checkIfNameDeviceAndGradeRegistered($avatarname, $deviceid, $grade);
            ...
            if (!$childexists) {
                $responsedata = array(
                    'objid' => $objid,
                    'status' => "failed",
                    'description' => "No account exists with the given name, deviceid and grade"
                    // 'description'  => "No account exists with the given name and deviceid"
                );
                $em = new exceptionMgr(" ");
                $em->logInfo("txabbchmgameplaydetail: Error: No account exists with the given name, deviceid and grade (" . $avatarname . "," . $deviceid . "," . $grade . ")");
            } else {

                // $childid = getChildIdByNameAndDevice($avatarname, $deviceid);
                $childid = getChildIdByNameDeviceAndGrade($avatarname, $deviceid, $grade);

                if ($childid == null) {
                    $responsedata = array(
                        'objid' => $objid,
                        'status' => "failed",
                        'description' => "Could not retrieve the Child ID for the given name, deviceid and grade"
                        // 'description' => "Could not retrive the Child ID for the given name and deviceid"
                    );
                    $em = new exceptionMgr(" ");
                    $em->logInfo("txabbchmgameplaydetail: Error: Could not retrieve the Child ID for the given name, deviceid and grade (" . $avatarname . "," . $deviceid . "," . $grade . ")");
                } else {
