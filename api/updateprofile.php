...
if ($data) {
    ...
    if ($childexists) {
        ...
        if (!$childid) {
            $responsedata = array(
                'status' => "failed",
                'description' => "Given combination of name and device, grade ($childname, $deviceid, $grade) does not exist."
            );
            $em = new exceptionMgr(" ");
            $em->logInfo("updateprofile: Error: Given combination of name and device, grade ($childname, $deviceid, $grade) does not exist.");
        } else {
