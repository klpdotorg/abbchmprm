...
if ($childname && $deviceid && $grade) {

    // $rtn = checkIfNameAndDeviceRegistered($childname, $deviceid);
    $rtn = checkIfNameDeviceAndGradeRegistered($childname, $deviceid, $grade);


    if (!$rtn) {
        $responsedata = array(
            'status' => "failed",
            'description' => "No Child is registered with the given name, deviceid and grade ($childname, $deviceid, $grade)."
            // 'description' => "No Child is registered with the given name and deviceid ($childname, $deviceid)."
        );
        $em = new exceptionMgr(" ");
        $em->logInfo("getchild: Error: No Child is registered with the given name, deviceid and grade ($childname, $deviceid, $grade).");
    } else {
        ...
        if (!$objChild) {
            ...
            if (!$objChild_userprogress) {
                ...
                $em = new exceptionMgr(" ");
                $em->logInfo("getchild: Error: Failed to retrieve the Child details for the given name, deviceid and grade ($childname, $deviceid, $grade)");
            } else {
