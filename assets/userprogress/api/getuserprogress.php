...
$appbasedir = substr($appbasedirorg, 0, -4);
...
if ($childname && $deviceid) {
    ...
    if ($grade && function_exists('getChildByNameAndDeviceAndGrade')) {
        ...
        if (!$rtn) {
            ...
            $em = new exceptionMgr(" ");
            $em->logInfo("getuserprogress: Error: No Child is registered with the given name and deviceid ($childname, $deviceid).");
    ...
    if (!$objChild) {
        $responsedata = array(
            'status' => "failed",
            'description' => "Failed to retrieve the Child details for the given name, deviceid and grade ($childname, $deviceid, $grade)."
        );
        $em = new exceptionMgr(" ");
        $em->logInfo("getuserprogress: Error: Failed to retrieve the Child details for the given name, deviceid and grade ($childname, $deviceid, $grade).");
    } else {
        ...
        if (!$objChild_userprogress) {
            ...
            $em = new exceptionMgr(" ");
            $em->logInfo("getuserprogress: Error: Failed to retrieve the Child user progress details for the given child");
        } else {

            $resolvedGrade = "";
            if (method_exists($objChild, 'getGradeName')) {
                $resolvedGrade = $objChild->getGradeName();
            } else {
                $resolvedGrade = $grade;
            }

            $isBBPP = (
                $resolvedGrade == "6th Grade"
            );
            ...
            if ($isBBPP) {
                ...
                $responsedata = array(
                    ...
                    'PMNSST' => $objChild_userprogress->getPMNSST(),
                    'CMNSST' => $objChild_userprogress->getCMNSST(),

                    'PMMST' => $objChild_userprogress->getPMMST(),
                    'CMMST' => $objChild_userprogress->getCMMST(),

                    'PMNOST' => $objChild_userprogress->getPMNOST(),
                    'CMNOST' => $objChild_userprogress->getCMNOST(),

                    'PNS' => $objChild_userprogress->getPNS(),
                    'PNO' => $objChild_userprogress->getPNO(),
                    'PM' => $objChild_userprogress->getPM(),
                    'PSG' => $objChild_userprogress->getPSG(),
                    'PDH' => $objChild_userprogress->getPDH(),
                    'PNSN' => $objChild_userprogress->getPNSN(),
                    'PNSF' => $objChild_userprogress->getPNSF(),
                    'PNSS' => $objChild_userprogress->getPNSS(),
                    'PNSC' => $objChild_userprogress->getPNSC(),
                    'PNSPV' => $objChild_userprogress->getPNSPV(),
                    'PML' => $objChild_userprogress->getPML(),
